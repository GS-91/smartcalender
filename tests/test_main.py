...
def test_create_event(monkeypatch):
    def dummy_event(user_input, day, time, duration):
        return {"message": "Dummy-Termin erstellt", "link": "http://example.com"}

    monkeypatch.setattr(main, "create_calendar_event", dummy_event)

    response = client.post("/calendar/create_event/", json={
    ...
    monkeypatch.setattr(main, "get_calendar_service", lambda: type("DummyService", (), {
        "events": lambda self=None: type("obj", (), {"list": lambda *a, **kw: type("r", (), {"execute": dummy_list})()})
    })())
...
def test_list_calendars(monkeypatch):
    ...
    import app.main as main
    monkeypatch.setattr(main, "get_calendar_service", lambda: type("DummyService", (), {
        "calendarList": lambda self=None: type("obj", (), {"list": lambda *a, **kw: type("r", (), {"execute": dummy_calendars})()})
    })())
